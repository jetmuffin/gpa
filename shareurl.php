<?php 
	function shareUrlWeibo($gpa)
	{
		...
		$pic = 'http://gpaquery.sinaapp.com/images/logo.png';
		$pic = urlencode($pic);
	 	$share_url_weibo = 'http://service.weibo.com/share/share.php?url='.$url.'&pic='.$pic.'&type=icon&language=zh_cn&appkey=5gBsE5&title='.$title.'&searchPic=true&style=simple';		
	...
	function shareUrlRenren($gpa)
	{
		...
		$pic = 'http://gpaquery.sinaapp.com/images/logo.png';
		$pic = urlencode($pic);
		$share_url_renren = 'http://widget.renren.com/dialog/share?resourceUrl='.$url."&title=".$title."&description=".$description.'&pic='.$pic;
	...
	function shareUrlQzone($gpa)
	{
		...
		$share_url_qzone = 'http://sns.qzone.qq.com/cgi-bin/qzshare/cgi_qzshare_onekey?url='.$url.'&title='.$title.'&summary='.$summary.'&pics='.$pic;
